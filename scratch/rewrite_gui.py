...
with open('main_gui_lkr.py', 'r', encoding='utf-8') as f:
    code = f.read()

prefix_idx = code.find('def show_results(')
if prefix_idx == -1:
    print('Error: def show_results not found')
    sys.exit(1)

prefix_code = code[:prefix_idx]

new_function = """def show_results(self, final_verdict, message, statuses, images, elapsed_time_ms):
        ...
        verdict_color = "green" if final_verdict else "red"
        Label(master=verdict_frame, text=f"LKR Robust Verdict: {message}", fg=verdict_color, font="Verdana 14 bold").grid(row=1, column=1, padx=20)
        Label(master=verdict_frame, text=f"Processing Speed: {elapsed_time_ms:.1f} ms", fg='black', font="Verdana 11").grid(row=1, column=2, padx=20)
...
with open('main_gui_lkr.py', 'w', encoding='utf-8') as f:
    f.write(prefix_code + new_function)
