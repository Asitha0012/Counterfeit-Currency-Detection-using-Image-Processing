...
# %store -r confidence_score
# %store -r flat_verdict
# %store -r veto_verdict
# %store -r elapsed_time_ms

# Safety Try-Except fallbacks for variables to prevent NameError exceptions during testing
try:
    _ = path
except NameError:
    path = ""
...
def display_output():
    ...
    for i in range(4):
        for j in range(3):
            ...
            if feature_num < 12:     # There are 12 features
                ...
                if feature_num < 7:
                    ...
                    status = result_list[feature_num][1]
                    if status == True:
                        text2 = "Base color & thread shift verified!"
                    else:
                        text2 = "Invalid color/thread profiles!"
                else:
                    status = result_list[feature_num][1]
                    if status == True:
                        text2 = "Watermark texture verified!"
                    else:
                        text2 = "Invalid or faint watermark!"
    ...
    # Dual-verdict classification logic
    is_flat_genuine = (pass_count >= 10)
    feat11_passed = False
    if len(result_list) > 10:
        feat11_passed = (result_list[10][1] == True) # Feature 11 is color/thread check at index 10
        
    is_veto_genuine = is_flat_genuine and feat11_passed
    current_confidence = (pass_count / 12.0) * 100.0
    
    # Render Dual-Verdict Results in sub_frame3
    header_text = f"ANALYSIS COMPLETE: {pass_count} / 12 Features Passed (Confidence Score: {current_confidence:.1f}%)"
    lbl_header = Label(master=sub_frame3, text=header_text, fg = 'dark blue', font = "Verdana 16 bold", pady=5)
    lbl_header.pack()
    
    # Frame for side-by-side or stacked verdicts
    ...
    lbl_flat = Label(master=verdict_frame, text=f"Baseline Verdict (Flat Voting): {flat_txt}", fg=flat_fg, font="Verdana 14 bold")
    lbl_flat.grid(row=1, column=1, padx=20)
    
    veto_txt = "GENUINE" if is_veto_genuine else "COUNTERFEIT"
    veto_fg = "green" if is_veto_genuine else "red"
    lbl_veto = Label(master=verdict_frame, text=f"Robust Verdict (Veto-Based): {veto_txt}", fg=veto_fg, font="Verdana 14 bold")
    lbl_veto.grid(row=1, column=2, padx=20)
    
    # Extra hint message about veto gates
    hint_text = "Note: Veto-Based classification requires Feature 11 (Security Thread/Color) to pass regardless of other features."
    lbl_hint = Label(master=sub_frame3, text=hint_text, fg='gray', font="Verdana 10 italic", pady=5)
    lbl_hint.pack()
    
    if elapsed_time_ms > 0:
        lbl_time = Label(master=sub_frame3, text=f"Processing Speed: {elapsed_time_ms:.1f} ms", fg='black', font="Verdana 11 bold")
        lbl_time.pack()
